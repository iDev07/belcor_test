...
import { ManipulatorState, Sample, CommandHistory } from '../types/types';
  ...
  reducers: {
    ...
    grabSample: (state) => {
      if (!state.hasGrabbedSample) {
        const sample = state.samples.find(
          (s) => s.position.x === state.position.x && s.position.y === state.position.y
        );
        if (sample) {
    ...
    releaseSample: (state) => {
      if (state.hasGrabbedSample && state.grabbedSampleId !== null) {
        const sample = state.samples.find((s) => s.id === state.grabbedSampleId);
        if (sample) {
